...
2. design.md の内容から関連するタグを判断し、knowledge を読む:
   `node .claude/tools/knowledge.mjs read knowledge/pitfalls.jsonl -tags "{関連タグ}"`
   `node .claude/tools/knowledge.mjs read knowledge/domain.jsonl -tags "{関連タグ}"`
3. plan.md に基づいてコードを実装する
4. 完了をチームリードに報告する
...
コードレビュー指摘への修正や、チームリードからの設計変更対応で自分のミスに気づいた場合、再発しそうなものは `knowledge/pitfalls.jsonl` に記録する:

```bash
node .claude/tools/knowledge.mjs add knowledge/pitfalls.jsonl -key "{識別名}" -insight "{何を間違えて、正しくはどうすべきか}" -source "observed" -tags "{タグ}"
```
